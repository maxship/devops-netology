...
7. Обладая знаниями о том, как штатным образом завершается соединение (FIN от инициатора, FIN-ACK от ответчика, ACK от инициатора), опишите в каких состояниях будет находиться TCP соединение в каждый момент времени на клиенте и на сервере при завершении. Схема переходов состояния соединения вам в этом поможет.
    ...
    1. Клиент завершает соединение, отправив сегмент с флагом FIN. Переходит в состояние FIN-WAIT-1. 
    2. Сервер получает сегмент и отправляет, в свою очередь сегмент с флагом ACK. Переходит в состояние CLOSE-WAIT и продолжает передачу данных.
    3. Клиент получает сегмент ACK и продолжает прием от сервера, ожидая сегмент с флагом FIN. Состояние соединения FIN-WAIT-2.
    4. Сервер посылает сегмент с флагом FIN и заканчивает передачу. Состояние LAST-ACK.
    5. Клиент получает сегиент FIN и отправляет сегмент с флагом ACK. Состояние TIME-WAIT. Ожидание в течение `2*MSL` и закрытие соединения.
