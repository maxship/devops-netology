...
Исходя из логики работы протокола TCP, для того чтобы можно было послать пакет с флагом закрытия соединения FIN, оно сначала должно быть установлено. А это, в свою очередь, требует выполнения трехэтапного "рукопожатия" с использования флагов SYN. Поэтому при штатной работе сети оба этих флага не могут находиться в одном пакете.
