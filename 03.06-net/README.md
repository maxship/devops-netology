...
7. Обладая знаниями о том, как штатным образом завершается соединение (FIN от инициатора, FIN-ACK от ответчика, ACK от инициатора), опишите в каких состояниях будет находиться TCP соединение в каждый момент времени на клиенте и на сервере при завершении. Схема переходов состояния соединения вам в этом поможет.
    ...
    - Клиент завершает соединение, отправив сегмент с флагом FIN. Переходит в состояние FIN-WAIT-1. 
    - Сервер получает сегмент и отправляет, в свою очередь сегмент с флагом ACK. Переходит в состояние CLOSE-WAIT и продолжает передачу данных.
    - Клиент получает сегмент ACK и продолжает прием от сервера, ожидая сегмент с флагом FIN. Состояние соединения FIN-WAIT-2.
    - Сервер посылает сегмент с флагом FIN и заканчивает передачу. Состояние LAST-ACK.
    - Клиент получает сегиент FIN и отправляет сегмент с флагом ACK. Состояние TIME-WAIT. Ожидание в течение `2*MSL` (Maximum Segment Lifetime) и закрытие соединения.

![1280px-Tcp_state_diagram_fixed svg](https://user-images.githubusercontent.com/72273610/123435343-4b2be500-d5ef-11eb-85da-8cfedc82b829.png)


8. TCP порт – 16 битное число. Предположим, 2 находящихся в одной сети хоста устанавливают между собой соединения. Каким будет теоретическое максимальное число соединений, ограниченное только лишь параметрами L4, которое параллельно может установить клиент с одного IP адреса к серверу с одним IP адресом? Сколько соединений сможет обслужить сервер от одного клиента? А если клиентов больше одного?

Не уверен, но теоретически, количество соединений клиент-сервер будет равно количеству портов `2^16 = 65536` (правда часть из них зарезервирована системой, поэтому по факту будет чуть меньше). В случае, если клиентов больше одного, то `65к * количество подключенных IP`.

9. Может ли сложиться ситуация, при которой большое число соединений TCP на хосте находятся в состоянии  `TIME-WAIT`? Если да, то является ли она хорошей или плохой? Подкрепите свой ответ пояснением той или иной оценки.

Насколько я понимаю, большое количество соединений в состоянии `TIME-WAIT` возможно, и в ряде ситуаций это абсолютно нормально, если действительно создается большое количество недолгоживущих соединений. И состояние `TIME-WAIT` просто говорит о том, что сокет будет ждать время, равное `2*MSL`, а по его прошествии закроется.

С другой стороны, если новые соединения устанавливаются быстрее, чем закрываются старые, то очередь TIME-WAIT будет расти и в итоге переполнится. При этом простого способа уменьшить TIME-WAIT не существует, поэтому этот момент нужно учитывать при разработке сервисов.
