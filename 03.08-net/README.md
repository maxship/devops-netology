...
Формулировка задания не понятна. Правильно ли я понял, что в рассматриваемой схеме нет деления на MASTER и BACKUP, а вместо этого на каждый хост привязан отдельный VIP?  
В таком случае, если используется 3 VIP адреса, то трафик делится на 3 канала по 500 Мбит/с. Соответственно, при потере одного из них, на одном из каналов будет трафик в 1 Гбит/с, что даст огромные потери. Поэтому минимальное число адресов VIP будет равно 4, т.к. в этом случае на 2 канала пойдет по 375 Мбит/с и еще на один 750 Мбит/с.
