...
- Данные записываются на все узлы с задержкой до часа (асинхронная запись). Судя по длительной задержке синхронизации, это скорее всего PA система, т.к. корректность данных в нормальном режиме работы не всегда гарантирована. По PACELC это PA/EL система (где скорость важнее корректности данных).
- При сетевых сбоях, система может разделиться на 2 раздельных кластера. PA по CAP теореме, т.к. не обеспечивается корректность данных при разделении, но оба кластера работают и доступны. По PACELC это PA/EL система, которая в условиях отсутствия разделения обеспечивает минимальное время ответа. Либо PA/EС, которая в отсутствие проблем с сетью обеспечивает консистентность.
- Система может не прислать корректный ответ или сбросить соединение. Система плохо переносит разделение, поэтому это PC. Про скорость ответа ничего неизвестно, по PACELC это может быть PC/EC или PC/EL.
